...
#num_of_samples = input("Please input number of samples>> ")
...
groupby_time = df.groupby('timestamp')
...
#Create Document
vectorizer = CountVectorizer(analyzer = 'word', min_df = 5, stop_words = 'english', lowercase = True, token_pattern = '[a-zA-z0-9]{3,}', max_features = 50000)
...
document = ['Document' + str(i) for i in range(len(data))]

#Topic - Score Dataframe
df_document_topic = pd.DataFrame(np.round(lda_output,2), columns=topicnames, index = document)
df_document_topic1 = df_document_topic.head(15)
#print(df_document_topic.head())
details = pd.DataFrame(np.round(lda_output,3), columns= topicnames, index = document)
sorted_details = details.sort_values(by='Document0', axis=1, ascending=False)
...
{
 "topics":[
     print(df_document_topic1.to_json(orient='index'))
     
     ],
 "trends":
     print(data_trend.to_json(orient = 'index'))
 } 
